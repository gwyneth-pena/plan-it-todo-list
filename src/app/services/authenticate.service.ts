...
export class AuthenticateService {
  ...
  login(loginFormData: any): Observable<any> {
    return this.http
      .post<any>(`${this.url}/login`, loginFormData, this.httpOptions)
      .pipe(catchError(this.errorHandlerService.errorHandler));
  }

  signUp(signUpFormData: User): Observable<User> {
    return this.http
      .post<User>(`${this.url}/signup`, signUpFormData, this.httpOptions)
      .pipe(catchError(this.errorHandlerService.errorHandler));
  }

  getCurrentUser(loggedIn: any): Observable<any> {
    return this.http
      .get(`${this.url}/currentUser/${loggedIn}`, this.httpOptions)
      .pipe(catchError(this.errorHandlerService.errorHandler));
  }
}
