...
export class TaskService {
  ...
  private url = 'https://plan-it-todo.herokuapp.com/tasks';
  ...
  addTask(taskObject:any):Observable<Task>{
        return this.http.post<Task>(`${this.url}/add`,taskObject,this.httpOptions).pipe(
  ...
  deleteTask(task_id:number):Observable<any>{
       ...
       return this.http.delete<any>(`${this.url}/delete/${task_id}`).pipe(
         catchError(this.errorHandler.errorHandler)
       )
  ...
  updatedCompleted(values:any){
     return this.http.patch(`${this.url}/updateCompleted`,values,this.httpOptions).pipe(
        catchError(this.errorHandler.errorHandler)
     )
  }

  editTask(values:any):Observable<any>{
      return this.http.patch(`${this.url}/update`,values,this.httpOptions).pipe(
        catchError(this.errorHandler.errorHandler)
 ...
 convertTimeToTwelve(value:any){
  ...
  if(hourNum==0){hours='12'; amOrPM='AM'};
  if(hourNum==12){hours='12'; amOrPM='PM'};
